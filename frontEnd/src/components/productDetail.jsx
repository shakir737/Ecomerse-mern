...
const cartDetail = (users) => {
  ...
  if(users){
    if(users.getaUser.cart.length > 0) {
      users.getaUser.cart.map((i, index) => {
        ...
        const result = productState.filter((product) => product._id === i.product)
 ...
 // const isItemExists = cart && cart.find((i) => i._id === id)
   const addToCart = async (_id, userID, order) => {
    ...
    if(_id && status[0] !== true){
      ...
      alert("Please Enter Less Quantity Than Available Stock");
    }   
   };
  ...
  const wishlistfilter = () => {
    const data =  users.getaUser.wishlist.filter(product => product === detail);
    return data;
  }
            ...
            <img src={`${productInfo[0].imageUrls[0]}`} alt="" className="w-[500px] h-[480px]" /> ):(
              <img src={`${imageUrl}`} alt="" className="w-[500px] h-[480px]" /> 
                ...
                <div className="border-[8px] shadow rounded-md">
                 ...
                 <span className="items-center"> Messages</span></div>
                 ...
                 // className={`${styles.button} border mt-4 rounded-[4px] `}
                  ...
                  <input
                      ...
                      value={message}
                    />
                    <button  type="submit" className={`absolute right-0 h-[38px] mt-[1px] rounded-full cursor-pointer ${styles.button}`} >submit</button>
                    </div>
                    </form>
                  </div>
                  </div>
