...
const handleRemoveFromCart = async(item) => {
  ...
  return (
    <div to={`/?id=${item._id}`} className="card shadow-xl relative mr-5 md:my-5">
      <div
      ...
      <div className="flex mx-auto max-h-full">
       {item ? (
                   ...
                   <Link to={`/?id=${item._id}`}><spain className="font-bold" >{item.title}</spain></Link>
        <p>{item.description}</p>
        <div className="card-actions justify-between items-center mt-2">
          <h5 className="font-semibold">
            <span className="text-sm text-red">$ </span> {item.productDetail[0].price}
          </h5>
         { inCart ? (
          <button onClick={() => handleRemoveFromCart(item)} className="btn bg-red text-white">Remove From Cart </button>
